...
async def receive_session(request):
    ...
    console.print(f"  [cyan]Ready to scan! Run:[/cyan]")
    console.print(f"  python suika.py scan -t https://{domain} -m redstorm --session {filename}\n")
...
def run_server(host: str = "127.0.0.1", port: int = 9999):
    ...
    console.print(f"[dim]Waiting for browser extension data...[/dim]\n")
    console.print("[bold]Endpoints:[/bold]")
    console.print(f"  GET  /health          - Health check")
    console.print(f"  POST /capture         - Receive live captures")
    console.print(f"  POST /session         - Receive full session export")
    console.print(f"  GET  /session/{{domain}} - Get session for domain")
    console.print(f"  GET  /sessions        - List all sessions")
    console.print(f"  POST /scan            - Trigger scan with captured session")
    console.print()
